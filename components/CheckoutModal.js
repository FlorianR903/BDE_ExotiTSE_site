...
// Stripe chargé uniquement si la clé publique existe
...
export default function CheckoutModal({ clientSecret, onClose }) {
    ...
    /** ⚠️ Fallback 1 : aucune clé Stripe publique */
    if (!process.env.STRIPE_PUBLISHABLE_KEY) {
        return createPortal(
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                <div className="bg-white p-8 rounded-2xl max-w-md text-center">
                    <h3 className="text-xl font-bold text-red-500 mb-2">Paiement indisponible</h3>
    ...
    /** ⚠️ Fallback 2 : le serveur n’a pas renvoyé de clientSecret */
    if (!clientSecret) {
        return createPortal(
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                <div className="bg-white p-8 rounded-2xl max-w-md text-center">
                    ...
                    <p className="text-gray-700">
                        Impossible de démarrer le paiement (clientSecret manquant).
                    </p>
                    <button
                        onClick={handleClose}
